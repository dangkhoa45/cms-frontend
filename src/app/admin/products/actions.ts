...
export async function createProductAction(data: CreateProductDto) {
  try {
    ...
    const response = await backendApi.admin.products.create(
      data,
      cookieHeader
    );
    ...
    return {
      success: false,
      error: formatActionError(error, 'Failed to create product'),
    };
  }
}
...
export async function updateProductAction(
  ...
  try {
    ...
    const response = await backendApi.admin.products.update(
      id,
      data,
      cookieHeader
    );
    ...
    return {
      success: false,
      error: formatActionError(error, 'Failed to update product'),
    };
  }
}
...
export async function deleteProductAction(id: string) {
  try {
    ...
    await backendApi.admin.products.remove(id, cookieHeader);

    return { success: true };
  } catch (error) {
    return {
      success: false,
      error: formatActionError(error, 'Failed to delete product'),
    };
  }
}
